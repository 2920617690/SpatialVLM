...
def run_pointer_stage(config_path: str | Path) -> None:
    ...
    trainable = sum(p.numel() for p in model.pointer.parameters())
    print(f"[pointer] stage={config['stage']} trainable_pointer_params={trainable}")
    print(f"[pointer] manifest={config['data']['train_manifest']}")
