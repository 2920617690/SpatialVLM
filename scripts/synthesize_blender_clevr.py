...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Synthesize Blender CLEVR-style QCR training data.")
...
def _run_blender(config_path: Path, config: dict) -> None:
    ...
    render_script = ROOT / "scripts" / "blender_clevr_render.py"
    subprocess.run(
        [
            blender_binary,
            "--background",
    ...
    for split, expected_scene_count in config["splits"].items():
        ...
        for metadata_path in metadata_paths[:expected_scene_count]:
            ...
            for q_index in range(q_per_scene):
                ...
                sample.metadata["render_backend"] = "blender_clevr"
        ...
        print(
            f"[blender-clevr] split={split} scenes={expected_scene_count} "
